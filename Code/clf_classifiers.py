...
def classify_vegetation(points, radius, threshold):
    """
    Classify vegetation points into low vegetation and trees based on the
    height difference in a cylindrical neighbourhood around each point.
    ...
    threshold : float or int
        The height difference separating low vegetation and trees.

    Returns
    -------
    classification : list
        The resulting classification into 'tree' and 'low_veg' for all the
        points.
    """
    kdtree = cKDTree(points[:, :2])
    classification = []
    for p in points:
        neighbours = kdtree.query_ball_point(p[:2], radius)
        local_points = points[neighbours]
        delta_z = max(local_points[:, 2]) - min(local_points[:, 2])
        if delta_z > threshold:
            classification.append('tree')
        else:
            classification.append('low_veg')
